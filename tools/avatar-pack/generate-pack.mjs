...
const outDir = resolve(process.cwd(), arg('--out-dir', './tmp/avatar-pack-batch'));
const model = arg('--model', 'gpt-image-1');
const dryRun = process.argv.includes('--dry-run');

const presets = JSON.parse(readFileSync(promptsPath, 'utf8')).presets || [];
if (!presets.length) {
  console.error('No presets found in prompts.json');
  process.exit(1);
}
